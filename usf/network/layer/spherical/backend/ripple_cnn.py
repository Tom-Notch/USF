#!/usr/bin/env python3
#
# Created on Tue Feb 18 2025 17:32:20
...
class RippleConv(GenericSphericalConv):
    """Instead of discretizing the circle into finite number of rings
    Can we use a 1D MLP to weight inputs based on geodesic distance?
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            weighting_function_config (dict[str, dict[str, Any]]): Weighting function config for distance branch.
            ...
            backend (str, optional): to swallow backend kwarg from parent class initialization. Defaults to "ripple".
        """
        assert (
            backend == "ripple"
        ...
        ), "Must use continuous function for distance in RippleConv"
        assert all(
            dim > 0 for dim in weighting_function_config["distance"]["hidden_dims"]
        ...
        weighting_function_config["distance"]["in_channels"] = 1
        weighting_function_config["distance"]["out_channels"] = 1
        weighting_function_config["distance"]["num_functions"] = (
        ...
        self.MLPs = WeightingFunction(weighting_function_config["distance"])
    ...
    def reset_parameters(self) -> RippleConv:
        """Reinitialize bias (uniform, scaled by kernel radius).

        Returns:
            RippleConv: ``self`` for chaining.
        """
    ...
    def _block_collection_matrix(
        ...
        Returns three tensors, each of shape (num_output_pixels, num_max_candidates):
        - padded_indices: candidate input indices, shape (num_output_pixels, K)
        - padded_distances: candidate geodesic distances, shape (num_output_pixels, K, 1)
        - mask: boolean mask indicating valid candidate positions, shape (num_output_pixels, K)
        """
        assert (
        ...
        if len(membership_indices_list) == 0:
            padded_indices = torch.zeros(
                (num_output_pixels, 1), dtype=torch.int64, device=_device
            )
            padded_geodesic_distances = torch.zeros(
                (num_output_pixels, 1, 1), dtype=_dtype, device=_device
            )
            mask = torch.zeros((num_output_pixels, 1), dtype=torch.bool, device=_device)
            return padded_indices, padded_geodesic_distances, mask
        ...
        return padded_indices, padded_geodesic_distances.unsqueeze(-1), mask
    ...
    def _get_collection_matrix(
        ...
        - padded_indices, shape (num_output_points, K)
        - padded_distances, shape (num_output_points, K, 1)
        - mask, shape (num_output_points, K)
        ...
        if (
            ...
            padded_indices, padded_distances, mask = self.collection_matrix_cache[
                input_output_hash
            ]
            if is_cache_enabled():
            ...
            # Both are missing, so compute and cache the result
            padded_indices, padded_distances, mask = self._block_collection_matrix(
    ...
    def _query_weights(
        ...
        """
        Query MLPs weights for the candidates based on geodesic distances.
        ...
        candidate_weights[mask] = checkpoint(
            self.MLPs,
            padded_distances[mask],
            use_reentrant=False,
        ).squeeze(-1)
        # Reshape to (num_output_pixels, num_max_candidates, in_channels, out_channels)
    ...
    def _block_query_weights(
        ...
        for distance_chunk, mask_chunk in zip(distance_chunks, mask_chunks):
            candidate_weight_chunks.append(
                self._query_weights(distance_chunk, mask_chunk)
            )
    ...
    def _get_candidate_weights(
        ...
        """
        Compute candidate weights based on geodesic distances using the MLP in training mode.
    ...
    def forward(
        ...
        # 3. Compute the collection matrix.
        padded_indices, padded_distances, mask = self._get_collection_matrix(
        ...
        candidate_weights = self._get_candidate_weights(padded_distances, mask)
    ...
    def report_metrics(self, return_metrics: bool = False) -> dict | None:
        """Report statistics on the latest collection matrix for RippleConv.
        ...
        # (padded_indices, padded_distances, mask)
        padded_indices, padded_distances, mask = self.latest_collection_matrix
    ...
    def visualize_kernel(
        ...
        ripple_height: float = 0.005,
        colormap: str = "hsv",
    ) -> SphericalImage:
        """
        Visualize the kernel weights of RippleConv.
        ...
        Args:
            ...
            ripple_height (float, optional): height of ripple effect. Defaults to 0.005.
        ...
        # Elongate vectors
        elongation = 1.0 + ripple_height * visual_weight.reshape(
            1, -1, 1
...
register_cache("RippleConv.output_vector_cache", RippleConv.output_vector_cache)
register_cache("RippleConv.collection_matrix_cache", RippleConv.collection_matrix_cache)
