...
class WaveConv(GenericSphericalConv):
    """Building upon the idea of ripple cnn, can we make it directional and continuous?
    Yes of course! by having another 1D MLP that produces weights based on direction
    Or, the MLP could take input distance and direction, then output a single weight.
    These 2 options are different if you think about it.
    The first one is preferred because it respects the signal as signal on the sphere.
    Whereas the second one doesn't care.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            weighting_function_config (dict[str, dict[str, Any]]): Weighting function config for distance and direction branches.
            ...
            backend (str, optional): to swallow backend kwarg from parent class initialization. Defaults to "wave".
        """
        assert backend == "wave", f"wrong backend instantiated, got backend = {backend}"
        super().__init__()
        ...
        assert (
            weighting_function_config["distance"]["function"] == "continuous"
            and weighting_function_config["direction"]["function"] == "continuous"
        ), "Must use continuous weighting function for both distance and direction in WaveConv"
        assert all(
            dim > 0
            for dim in weighting_function_config["distance"]["hidden_dims"]
            + weighting_function_config["direction"]["hidden_dims"]
        ), "mlp_hidden_dims must be positive"
        ...
        distance_embedding = weighting_function_config["distance"].get("embedding")
        if (
            distance_embedding is not None
            and distance_embedding.get("type") == "cosine"
        ):
            distance_embedding["basis"] = float(math.floor(math.pi / self.radius))

        self.MLPs = MultiBranchWeightingFunction(weighting_function_config)
    ...
    def reset_parameters(self) -> WaveConv:
        """Reinitialize bias (uniform, scaled by kernel radius).

        Returns:
            WaveConv: ``self`` for chaining.
        """
    ...
    def _block_collection_matrix(
        ...
        - padded_distances: candidate geodesic distances, shape (num_output_pixels, num_max_candidates, 1).
        - padded_directions: candidate directions, shape (num_output_pixels, num_max_candidates, 1).
        ...
        return (
            padded_indices,
            padded_geodesic_distances.unsqueeze(-1),
            padded_directions.unsqueeze(-1),
            mask,
        )
    ...
    def _get_collection_matrix(
        ...
        The collection matrix consists of:
        - padded_indices: candidate input indices, shape (num_output_pixels, num_max_candidates).
        - padded_distances: candidate geodesic distances, shape (num_output_pixels, num_max_candidates, 1).
        - padded_directions: candidate directions, shape (num_output_pixels, num_max_candidates, 1).
        - mask: boolean mask indicating valid candidate positions, shape (num_output_pixels, num_max_candidates).
        ...
        padded_distances_buffer_name = f"padded_distances_{input_output_hash}"
        padded_directions_buffer_name = f"padded_directions_{input_output_hash}"
        ...
        padded_distances_buffer_entry = getattr(
            self, padded_distances_buffer_name, None
        )
        padded_directions_buffer_entry = getattr(
            self, padded_directions_buffer_name, None
        )
        ...
        if (
            ...
            (
                ...
                padded_distances_buffer_entry,
                padded_directions_buffer_entry,
                mask_buffer_entry,
            )
            ...
            if is_cache_enabled():
                ...
                self.register_buffer(padded_distances_buffer_name, padded_distances)
                self.register_buffer(
                    padded_directions_buffer_name,
                    padded_directions,
                )
        ...
        return (
            ...
            padded_distances.to(device=_device),
            padded_directions.to(device=_device),
            mask.to(device=_device),
        )

    def _query_weights(
        self,
        padded_distances: torch.Tensor,
        padded_directions: torch.Tensor,
        mask: torch.Tensor,
    ) -> torch.Tensor:
        """
        ...
        Args:
            padded_distances (torch.Tensor): Tensor of shape (num_output_pixels, num_max_candidates, 1).
            padded_directions (torch.Tensor): Tensor of shape (num_output_pixels, num_max_candidates, 1).
        ...
        """
        _device, _dtype = self.device, self.dtype
        num_output_pixels, num_max_candidates, _ = padded_distances.shape
        ...
        weights = checkpoint(
            self.MLPs,
            {
                "distance": padded_distances[mask],  # shape: (num_valid_distances, 1)
    ...
    def _block_query_weights(
        ...
        distance_chunks = torch.split(padded_distances, chunk_size, dim=0)
        direction_chunks = torch.split(padded_directions, chunk_size, dim=0)
        ...
        return torch.cat(candidate_weight_chunks, dim=0)
    ...
    def _get_candidate_weights(
        ...
        Args:
            padded_distances (torch.Tensor): Tensor of shape (num_output_pixels, num_max_candidates).
            padded_directions (torch.Tensor): Tensor of shape (num_output_pixels, num_max_candidates).
            mask (torch.Tensor): Boolean mask of shape (num_output_pixels, num_max_candidates) indicating valid candidates.
        ...
        if self.training:
            ...
            buffer_name = f"candidate_weights_{array_hash(padded_distances)}_{module_weights_hash(self.MLPs)}"
            candidate_weights = getattr(self, buffer_name, None)

            if candidate_weights is None:
                # clear old buffers
                old_buffer_prefix = f"candidate_weights_{array_hash(padded_distances)}_"
    ...
    def forward(
        ...
        Full forward pass of RippleConv:
            ...
            3. Compute the collection matrix:
                padded_indices, padded_distances, mask of shape (num_output_pixels, num_max_candidates).
        ...
        input_vectors = batch_spherical_image.vector.view(-1, 3)
        self.latest_input_vector = input_vectors.to(device=_device, dtype=_dtype)

        # 2. Compute output positions via location sampler.
        output_vectors = self._get_output_vector(input_vectors)
        self.latest_output_vector = output_vectors.to(device=_device, dtype=_dtype)
        ...
        candidate_weights = self._get_candidate_weights(
            padded_distances, padded_directions, mask
        )
    ...
    def report_metrics(self, return_metrics: bool = False) -> dict | None:
        """Report statistics on the latest collection matrix for WaveConv.
        ...
        # (padded_indices, padded_distances, mask)
        (
            padded_indices,
            padded_distances,
            padded_directions,
        ...
        valid_distances = padded_distances[mask]
        if valid_distances.numel() > 0:
            avg_distance = valid_distances.mean().item()
            std_distance = valid_distances.std(unbiased=False).item()
            max_distance = valid_distances.max().item()
            min_distance = valid_distances.min().item()
        else:
            avg_distance = std_distance = max_distance = min_distance = 0.0

        # Compute statistics for directions.
        valid_directions = padded_directions[mask]  # only valid candidates
        if valid_directions.numel() > 0:
            avg_direction = valid_directions.mean().item()
            std_direction = valid_directions.std(unbiased=False).item()
            max_direction = valid_directions.max().item()
            min_direction = valid_directions.min().item()
        else:
            avg_direction = std_direction = max_direction = min_direction = 0.0

        # Total number of queries to weighting function
        total_weighting_queries = mask.sum().item()

        print("Wave CNN Metrics:")
        print(
    ...
    def visualize_kernel(
        ...
        """
        Visualize the kernel weights of WaveConv.
        ...
        with torch.no_grad():
            distances = distances.to(device=_device, dtype=_dtype).view(-1, 1, 1)
            directions = directions.to(device=_device, dtype=_dtype).view(-1, 1, 1)
            total_points = distances.shape[0]
            candidate_weights = self._block_query_weights(
                distances,
                directions,
                torch.ones(
...
register_cache("WaveConv.output_vector_cache", WaveConv.output_vector_cache)
register_cache("WaveConv.collection_matrix_cache", WaveConv.collection_matrix_cache)
